...
import FileClipForm from "../../../../images/FileClipForm.svg";

export default function CollaborationForm() {
  const { data: session, status } = useSession();
  const [isLoading, setIsLoading] = useState(false);
  const [selectedPage, setSelectedPage] = useState(null);
  const [selectedPageId, setSelectedPageId] = useState(6);
  ...
  useEffect(() => {
    setSelectedPage(pagesArray[5]);

    const fetchLetters = async () => {
      try {
        const response = await fetch("/api/requests/get-requests");
        if (response.ok) {
          const data = await response.json();
          const filteredItems = data.data.filter(
            (item) => item.page === "title-agencies"
          );
          setLettersArray(filteredItems);
        } else {
    ...
    const fetchFields = async () => {
      try {
        const response = await fetch(
          "/api/collaboration-form/get-collaboration-form"
        );
        if (response.ok) {
          const data = await response.json();
          console.log(
            "data",
            data.data.filter((item) => item.page === "title-agencies")
          );
          const filteredItems = data.data.filter(
